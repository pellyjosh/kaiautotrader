...
# Set multiprocessing start method for systemd compatibility
try:
...
class PocketWorkerManager:
    ...
    def _start_workers(self):
        global_value.logger("[WorkerManager] _start_workers called", "DEBUG")
        if not self.configs:
        ...
        global_value.logger(f"[WorkerManager] Found {len(enabled_configs)} enabled configs", "DEBUG")
        ...
        for config in enabled_configs:
            name = config['name']
            global_value.logger(f"[WorkerManager] Starting worker for {name}...", "DEBUG")
            cmd_q = multiprocessing.Queue()
            resp_q = multiprocessing.Queue()
            global_value.logger(f"[WorkerManager] Created queues for {name}", "DEBUG")
            ...
            global_value.logger(f"[WorkerManager] Created process for {name}", "DEBUG")
            process.daemon = True  # Workers will exit if the main process exits
            global_value.logger(f"[WorkerManager] About to start process for {name}...", "DEBUG")
            process.start()
            global_value.logger(f"[WorkerManager] Process started for {name}", "DEBUG")
        ...
        global_value.logger("[WorkerManager] About to start trade result monitoring...", "DEBUG")
